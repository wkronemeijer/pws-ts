...
module TSP {
    ...
    export function deleteFrom<T>(array: T[], item: T) {
        let index = array.indexOf(item);
        if (index !== -1) {
    ...
    export function median(samples: number[]): number {
        let array  = samples.slice().sort((a, b) => a - b);
    ...
    export function swapIndices<T>(array: T[], index0: number, index1: number): T[] {
        let base = new Array<T>();
        
        let left   = array.slice(0, index0);
        let middle = array.slice(index0, index1).reverse();
        let right  = array.slice(index1);
